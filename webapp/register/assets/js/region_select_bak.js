...
names = [];
PCAA = [];
PCAN = arr.split("#"); // 北京市$市辖区,东城区,西城区,崇文区,宣武区,朝阳区,丰台区,石景山区,海淀区,门头沟区,房山区,通州区,顺义区,昌平区,大兴区,怀柔区,平谷区|市辖县,密云县,延庆县
for (i = 0; i < PCAN.length; i++) {
    ...
    privince[i] = PCAN[i].split("$")[0] + "," + TArea.join(",");
    city[i] = privince[i].split(",")[0];
    names[i] = privince[i].split(',')
}
...
function City(options) {
    this.selP = gid(options.province);
    this.selC = gid(options.city);
    this.selA = gid(options.area);
    this.DefP = options.vProvince;
    this.DefC = options.vCity;
    this.DefA = options.vArea;
    this._init();
}
...
City.prototype = {
    ...
    setP: function() {
		for (i = 0; i < city.length; i++) {
	        cityT = cityV = city[i];
	        if (cityT == SPT) cityV = "";
	        this.selP.options.add(new Option(cityT, cityV));
	        if (this.DefP == cityV) {
	        	this.selP[i].selected = true;
	        }
	    }
	    this.setC()
    },
    setC: function() {
    	PI = this.selP.selectedIndex;
	    this.selC.length = 0;
	    if (this.selP.value && ShowT) {
	        this.selC.options.add(new Option(SCT, ""))
	    }
	    for (i = 1; i < names[PI].length; i++) {
	        nameT = nameV = names[PI][i];
	        if (nameT == SCT) nameV = "";
	        this.selC.options.add(new Option(nameT, nameV));
	        if (this.DefC == nameV) this.selC[PCA.selP.value && ShowT ? i : i - 1].selected = true
	    }
	    if (this.selA) {
	    	this.setA();
	    }
    ...
    setA: function() {
    	PI = this.selP.selectedIndex;
	    CI = this.selC.selectedIndex;
	    this.selA.length = 0;
	    if (this.selC.value == "" && ShowT) {
	        this.selA.options.add(new Option(SAT, ""));
	        return
	    }
	    if (ShowT) {
	        this.selA.options.add(new Option(SAT, ""));
	    ...
	    for (i = 1; i < PCAA[PI][CI].length; i++) {
	        ...
	        if (PCAAT == SAT) PCAAV = "";
	        this.selA.options.add(new Option(PCAAT, PCAAV));
	        if (this.DefA == PCAAV) this.selA[ShowT ? i : i - 1].selected = true
	    }
    }
}
